const axios = require('axios');

const app = require("../../app");
const logger = require("../../logger");
const user_dao = require("../../dao/user");
const setupuser = require("../../util/setupuser");

const { MANAGEMENT, UPLOAD, OVERWRITE } = require("../../permissions");
const createjwt = require("../../util/createjwt");

app.get('/api/oauth_callback/mesehub', function(req, res){
  logger.debug("GET /api/oauth_callback/mesehub", req.query);

  const data = {
    client_id: process.env.MESEHUB_APP_ID,
    client_secret: process.env.MESEHUB_APP_SECRET,
    code: req.query.code,
		"grant_type": "authorization_code",
  	"redirect_uri": process.env.BASE_URL + "/api/oauth_callback/mesehub"
  };

  const options = {
    headers: {
			"Content-Type": "application/json",
      "Accept": "application/json"
    }
  };

	let user_info;
	axios.post("https://git.minetest.land/login/oauth/access_token", data, options)
  .then(r => {
    if (!r.data.access_token){
      console.log(r);
      throw new Error("No access_token received!");
    }
    console.log(r.data);
		return axios.get("https://git.minetest.land/api/v1/user", {
			headers: {
				"Authorization": "Bearer " + r.data.access_token
			}
		});
  })
	.then(r => {
 	 user_info = r.data;
 	 console.log(user_info);
 	 return user_dao.get_by_external_id(user_info.id);
 	 // user_info.login / avatar_url / name / email
  })
  .then(user => {
 	 if (user){
 		 if (user.type != "MESEHUB") {
 			 throw new Error("User already exists with another type!");
 		 }
 		 // valid github user
 		 return user;
 	 } else {
 		 // create new user
 		 return user_dao.create({
 			 name: user_info.login,
 			 type: "MESEHUB",
 			 hash: "", // no local password
 			 mail: user_info.email,
 			 external_id: user_info.id
 		 })
		 .then(setupuser);
 	 }
  })
  .then(user => {
		const token = createjwt(user, [UPLOAD, OVERWRITE, MANAGEMENT]);
 	 res.redirect(process.env.BASE_URL + "/#/oauth/" + token);
  })
  .catch(e => {
 	 console.log(e);
 	 res.send(e.message);
  });

});
