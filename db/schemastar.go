...
var schemaStarTable = ksql.NewTable("user_schema_star", "user_uid", "schema_id")
...
func (r *SchemaStarRepository) GetBySchemaAndUserID(schema_id int64, user_uid string) (*types.SchemaStar, error) {
	ss := &types.SchemaStar{}
	err := r.kdb.QueryOne(context.Background(), ss, "from user_schema_star where schema_id = $1 and user_uid = $2", schema_id, user_uid)
...
func (r *SchemaStarRepository) CountByUserUID(user_uid string) (int64, error) {
	c := &types.Count{}
	err := r.kdb.QueryOne(context.Background(), c, "select count(*) from user_schema_star uss join schema s on s.id = uss.schema_id where s.user_uid = $1", user_uid)
	return c.Count, err
}
